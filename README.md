...
单机个人知识库工具。当前仓库已清空旧 exe 实现，重新从底层架构设计开始。
...
- Runtime state is being consolidated under `AIstudyData` so the app can move between machines with its data folder.
- `AISTUDY_DATA_ROOT` can override the data root for portable or managed deployments.

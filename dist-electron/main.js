...
const chatgptWebLoginPort = Number(process.env.AISTUDY_CHATGPT_WEB_LOGIN_PORT
...
function getDeveloperDocumentDatabasePath() {
    return node_path_1.default.join(getDataDirectory(), developerDocumentDatabaseFile);
}
...
function getMimoConfigPath() {
    return node_path_1.default.join(getDataDirectory(), mimoConfigFile);
}
function getKnowledgeFormatDebugLogPath() {
    return node_path_1.default.join(getDataDirectory(), knowledgeFormatDebugLogFile);
}
...
async function appendKnowledgeFormatDebugLog(entry) {
    try {
        await promises_1.default.mkdir(getDataDirectory(), { recursive: true });
        const payload = {
        ...
        await promises_1.default.appendFile(getKnowledgeFormatDebugLogPath(), `${JSON.stringify(payload)}\n`, "utf8");
    }
    catch {
...
function getManagedPortRuntimeRoot() {
    ...
    return node_path_1.default.join(electron_1.app.getPath("userData"), "ChromeDidy");
}
function getRepositoryRootCandidates() {
    ...
    return [
        ...
        process.cwd(),
        node_path_1.default.resolve(exeDir, "..", ".."),
        node_path_1.default.resolve(process.resourcesPath, "..", "..", ".."),
...
function getManagedPortProfileDir(platform) {
    if (platform.id === "chatgpt") {
        return getChatGptLoggedChromeProfileDir();
    }
    return node_path_1.default.join(getManagedPortRuntimeRoot(), "chrome-profiles", `aistudy-${platform.id}-${platform.port}`);
}
function getChatGptLoggedChromeProfileDir() {
    ...
    return process.env.AISTUDY_CHATGPT_CHROME_USER_DATA_DIR
        ? localAppData
        : node_path_1.default.join(localAppData, "Google", "Chrome", "User Data");
}
function getChatGptLoggedChromeProfileName() {
    return process.env.AISTUDY_CHATGPT_CHROME_PROFILE || "Default";
}
function getManagedPortDefinition(platformId) {
    return managedPortPlatforms.find((platform) => platform.id === platformId);
}
function getManagedPortCdpUrl(port) {
    return `http://127.0.0.1:${port}`;
}
function getManagedPortKind(platform) {
    return platform.kind ?? "cdp";
}
async function fetchJsonWithTimeout(url, timeoutMs = 1200, init) {
...
async function getManagedPortStatus(platform) {
    ...
    try {
        const version = await fetchJsonWithTimeout(`${cdpUrl}/json/version`);
        const tabs = await fetchJsonWithTimeout(`${cdpUrl}/json`, 1200, {});
...
async function openManagedPortTab(platform) {
    const cdpUrl = getManagedPortCdpUrl(platform.port);
    try {
...
async function openUrlInCdpPort(port, url, hostKeyword) {
    const cdpUrl = getManagedPortCdpUrl(port);
    try {
        const tabs = await fetchJsonWithTimeout(`${cdpUrl}/json`, 1800, {});
...
async function waitForManagedCdpPort(port, waitMs = 8000) {
    const startedAt = Date.now();
    while (Date.now() - startedAt < waitMs) {
        if (await canConnectToPort("127.0.0.1", port, 700))
            return true;
        await new Promise((resolve) => setTimeout(resolve, 350));
    }
    return false;
}
async function openChatGptInLoggedChromePort(loginUrl) {
    ...
    const child = (0, node_child_process_1.spawn)(chromeExe, args, {
        detached: true,
        stdio: "ignore",
        windowsHide: false
    });
...
async function openChatGptLoginWindow(platform) {
...
async function startManagedPortService(platformId) {
...
async function openManagedLoginWindow(platformId) {
    const platform = getManagedPortDefinition(platformId);
    if (!platform) {
        throw new Error("未知端口平台");
    }
    ...
    const profileDir = getManagedPortProfileDir(platform);
    await promises_1.default.mkdir(profileDir, { recursive: true });
    await promises_1.default.writeFile(node_path_1.default.join(profileDir, "aistudy-port-profile.json"), JSON.stringify({
    ...
    const child = (0, node_child_process_1.spawn)(chromePath, [
...
async function getAiDailyWorkflowScriptPath() {
    const candidates = [
        ...
        electron_1.app.isPackaged ? node_path_1.default.join(process.resourcesPath, "scripts", "ai-daily-workflow.ps1") : "",
        node_path_1.default.join(__dirname, "..", "scripts", "ai-daily-workflow.ps1"),
        node_path_1.default.join(electron_1.app.getAppPath(), "scripts", "ai-daily-workflow.ps1")
    ].filter(Boolean);
    for (const candidate of candidates) {
        if (candidate && await fileExists(candidate))
            return candidate;
...
function sanitizeAiDailyRunRequest(request) {
...
async function readAiDailyManifest(manifestPath) {
    try {
...
async function getLatestAiDailyManifest() {
    const outputRoot = getAiDailyOutputRoot();
    try {
        const entries = await promises_1.default.readdir(outputRoot, { withFileTypes: true });
        const manifests = [];
        for (const entry of entries) {
            if (!entry.isDirectory())
                continue;
            const manifestPath = node_path_1.default.join(outputRoot, entry.name, "report", "manifest.json");
            try {
...
async function runAiDailyWorkflow(rawRequest) {
    const request = sanitizeAiDailyRunRequest((rawRequest || {}));
    ...
    if (request.zhihuUrl)
        args.push("-ZhihuUrl", request.zhihuUrl);
    if (request.query)
        args.push("-Query", request.query);
    if (request.forceTranscribe)
        args.push("-ForceTranscribe");
    if (request.skipDownloadTranscribe)
        args.push("-SkipDownloadTranscribe");
    return new Promise((resolve, reject) => {
        let stdout = "";
        let stderr = "";
        let settled = false;
        const child = (0, node_child_process_1.spawn)("powershell.exe", args, {
...
function isPathInsideDirectory(filePath, directory) {
    const resolvedFile = node_path_1.default.resolve(filePath).toLowerCase();
    const resolvedDirectory = node_path_1.default.resolve(directory).toLowerCase();
    return resolvedFile === resolvedDirectory || resolvedFile.startsWith(`${resolvedDirectory}${node_path_1.default.sep}`);
}
async function openAiDailyArtifact(rawPath) {
    ...
    const error = await electron_1.shell.openPath(targetPath);
    if (error)
...
async function startMysqlServerHidden(mysqldPath, iniPath, serverRoot, pidPath) {
...
function normalizeCollectionPayload(collection, payload, source) {
...
function getCollectionItems(payload, collection) {
    return payload?.[collection.payloadKey] ?? [];
}
async function loadLocalCollectionDatabase(collection) {
...
async function loadCollectionDatabaseFromMysql(collection) {
    return withMysqlConnection(async (connection) => {
        const [rows] = await connection.query(`SELECT payload_json FROM ${collection.tableName} ORDER BY position_index ASC, updated_at DESC`);
        const items = rows
            .map((row) => {
            try {
        ...
        if (items.length === 0)
            return null;
        const [metaRows] = await connection.query("SELECT meta_value FROM app_meta WHERE meta_key = ? LIMIT 1", [collection.savedAtMetaKey]);
        return {
            version: 1,
            source: "mysql",
...
async function loadCourseDatabase() {
    return loadCollectionDatabase(databaseCollectionIndex.courses);
}
async function loadDeveloperDocumentDatabase() {
    return loadCollectionDatabase(databaseCollectionIndex.developerDocuments);
}
async function saveCollectionItemsToMysql(collection, items) {
    if (!isCourseList(items))
        return;
...
async function saveCollectionDatabase(collection, items) {
    const databasePath = collection.databasePath();
    const backupPath = `${databasePath}.bak`;
...
async function saveDeveloperDocumentDatabase(documents) {
    await saveCollectionDatabase(databaseCollectionIndex.developerDocuments, documents);
}
function getClaudeSessionBindingsPath() {
    return node_path_1.default.join(getDataDirectory(), claudeSessionBindingsFile);
}
function getClaudeCourseWorkspacePath(courseId) {
    return node_path_1.default.join(getDataDirectory(), "claude-course-sessions", courseId.replace(/[^a-zA-Z0-9._-]/g, "_"));
}
async function loadClaudeSessionBindings() {
    try {
        const payload = await readJsonFile(getClaudeSessionBindingsPath());
        if (!payload || typeof payload !== "object" || Array.isArray(payload))
            return {};
        return Object.fromEntries(Object.entries(payload).filter((entry) => typeof entry[1] === "string"));
    }
    catch {
        return {};
    }
}
async function saveClaudeSessionBindings(bindings) {
    await promises_1.default.mkdir(getDataDirectory(), { recursive: true });
    await promises_1.default.writeFile(getClaudeSessionBindingsPath(), JSON.stringify(bindings, null, 2), "utf8");
}
async function requestClaudeBridgeJson(endpoint, body, timeoutMs = 900000) {
    const controller = new AbortController();
...
async function ensureClaudeBridgeReady() {
    ...
    try {
        const child = (0, node_child_process_1.spawn)(claudeAgentExePath, [], {
            cwd: node_path_1.default.dirname(claudeAgentExePath),
            detached: true,
            stdio: "ignore",
            windowsHide: true
        });
        ...
        throw new Error(`无法启动 Claude Code 会话：${error.message}`);
    }
...
function recoverClaudeText(value) {
    const decoded = iconv_lite_1.default.decode(iconv_lite_1.default.encode(value, "gb18030"), "utf8");
    const brokenScore = (text) => (text.match(/[\u0590-\u05ff\u0700-\u074f\ufffd]/g) ?? []).length;
    const readableScore = (text) => (text.match(/[\u4e00-\u9fff]/g) ?? []).length;
...
function extractJsonActionField(text) {
    const match = text.match(/"action"\s*:\s*"(none|replace_current_knowledge)"/);
    return match?.[1];
}
function extractAssistantTextFromJsonl(rawText) {
    const parts = [];
...
function normalizeClaudeReply(reply) {
    return {
        ...reply,
...
function extractJsonFromText(rawText) {
    ...
    if (!text)
        return null;
    try {
        return normalizeClaudeReply(JSON.parse(text));
    }
    catch {
    ...
    for (let end = text.lastIndexOf("}"); end >= 0; end = text.lastIndexOf("}", end - 1)) {
        for (let start = text.lastIndexOf("{", end); start >= 0; start = text.lastIndexOf("{", start - 1)) {
            ...
            try {
                return normalizeClaudeReply(JSON.parse(candidate));
            }
            catch {
                continue;
            }
...
async function readTextFileIfAvailable(filePath) {
    if (!filePath)
        return "";
    try {
...
async function readClaudeTaskText(task) {
    if (task.resultText?.trim())
...
async function startClaudeTrackedTask(payload) {
    const created = await requestClaudeBridgeJson("/api/claude/tasks", {
    ...
    if (!created.taskId)
        throw new Error("Claude Code MCP 创建失败");
    return created;
}
async function fetchClaudeTrackedTask(taskId) {
    const status = await requestClaudeBridgeJson(`/api/claude/tasks/${encodeURIComponent(taskId)}`, undefined, 8000);
    if (!status.ok)
        throw new Error(status.error || "Claude Code MCP 状态异常");
    return status;
}
function parseClaudeTaskEvent(rawEvent) {
...
async function waitForClaudeTaskEvent(taskId) {
    const controller = new AbortController();
...
async function waitForClaudeTrackedTask(taskId) {
    const deadline = Date.now() + 240000;
    while (Date.now() < deadline) {
        await new Promise((resolve) => setTimeout(resolve, 1200));
        const latest = await fetchClaudeTrackedTask(taskId);
        if (latest.completed || latest.status === "completed")
            return latest;
        if (latest.failed || latest.status === "failed") {
            const errorText = latest.error ||
...
function normalizeMimoBaseUrl(baseUrl) {
    return baseUrl.replace(/\/+$/, "");
}
async function loadMimoConfig() {
    ...
    const fileConfig = fromFile && typeof fromFile === "object" && !Array.isArray(fromFile)
        ? fromFile
        : {};
    const apiKey = fileConfig.apiKey?.trim() ||
...
function getMimoRequestTimeoutMs(mode) {
    return mode === "edit" ? 120000 : 45000;
}
function getMimoOutputBudget(mode) {
    return mode === "edit" ? 4096 : 768;
}
async function runMimoKnowledgeRequest(prompt, mode) {
...
function extractQuestionFromChatGptPrompt(prompt) {
...
async function runChatGptBrowserKnowledgeRequest(prompt, _mode) {
...
async function getDoubaoPageTarget(platform) {
    const cdpUrl = getManagedPortCdpUrl(platform.port);
    if (!await canConnectToPort("127.0.0.1", platform.port)) {
        throw new Error("Doubao 端口未启动，请先到信息搜集的端口管理打开 Doubao 登录窗口");
    }
    let targets = await fetchJsonWithTimeout(`${cdpUrl}/json`, 2500);
    ...
    if (!target || !target.url?.includes(platform.hostKeyword)) {
        ...
        await new Promise((resolve) => setTimeout(resolve, 1200));
        targets = await fetchJsonWithTimeout(`${cdpUrl}/json`, 2500);
...
function getDoubaoAutomationExpression(prompt, userMessage) {
    return `
(async () => {
...
async function getChatGptPageTarget(platform) {
    const cdpUrl = getManagedPortCdpUrl(platform.port);
    if (!await canConnectToPort("127.0.0.1", platform.port)) {
...
function getChatGptAutomationExpression(prompt, userMessage) {
    return `
(async () => {
  const prompt = ${JSON.stringify(prompt)};
  const userQuestion = ${JSON.stringify(userMessage)};
...
function getUserInstructionHead(message) {
    return message.split("【系统 README】")[0]?.slice(0, 800) || message.slice(0, 800);
}
function isKnowledgeEditRequest(message) {
...
async function handleAiChatRequest(request) {
    ...
    if (!Array.isArray(courses))
        throw new Error("课程数据不可用");
    const course = courses.find((item) => {
        return Boolean(item && typeof item === "object" && item.id === courseId);
    });
    if (!course)
        throw new Error("当前课程不存在");
    const knowledgePoints = course.knowledgePoints ?? {};
    const currentKnowledgeHtml = typeof knowledgePoints[nodeId] === "string" ? knowledgePoints[nodeId] : request.knowledgeHtml ?? "";
    ...
    const provider = request.provider === "chatgpt"
        ...
        : request.provider === "doubao"
            ...
            : request.provider === "mimo"
                ? "mimo"
                : "claude";
    const mimoMode = isKnowledgeEditRequest(message) ? "edit" : "answer";
    ...
    if (provider === "doubao") {
        ...
        await promises_1.default.mkdir(cwd, { recursive: true });
    ...
    if (parsed.action === "replace_current_knowledge" && typeof parsed.knowledgeHtml === "string") {
        ...
        if (!Array.isArray(latestCourses))
            throw new Error("课程数据不可用");
        const latestCourse = latestCourses.find((item) => {
            return Boolean(item && typeof item === "object" && item.id === courseId);
        });
        if (!latestCourse)
            throw new Error("当前课程不存在");
...
async function getSystemContextInfo() {
    ...
    const packageOutput = node_path_1.default.dirname(electron_1.app.getPath("exe"));
    const readmePath = node_path_1.default.join(repositoryRoot, "README.md");
    return {
        app: {
        ...
        paths: {
            ...
            claudeCourseSessionsDirectory: node_path_1.default.join(getDataDirectory(), "claude-course-sessions"),
            packageOutput,
            ...
            notionCache: node_path_1.default.join(electron_1.app.getPath("appData"), "Notion", "notion.db")
        },
        storage: {
        ...
        docs: {
            readme: readmePath,
            projectIndex: node_path_1.default.join(repositoryRoot, "PROJECT_INDEX.md"),
            updateLog: node_path_1.default.join(repositoryRoot, "src", "updateLog.ts"),
...
function createReleaseCheck(key, label, ok, detail) {
    return { key, label, ok, detail };
}
async function runRepositoryCommand(command, args, timeoutMs = 20000) {
    return new Promise((resolve) => {
...
async function getAppReleaseStatus() {
    const version = electron_1.app.getVersion();
    const tagName = `v${version}`;
    const releaseUrl = `${updateReleasePageUrl}/tag/${tagName}`;
    const packageJsonPath = node_path_1.default.join(repositoryRoot, "package.json");
    const updateLogPath = node_path_1.default.join(repositoryRoot, "src", "updateLog.ts");
    const latestYamlPath = node_path_1.default.join(repositoryRoot, "release", "latest.yml");
    const installerPath = node_path_1.default.join(repositoryRoot, "release", `AIstudy-Setup-${version}.exe`);
    ...
    let latestYaml = "";
    try {
        packageVersion = JSON.parse(await promises_1.default.readFile(packageJsonPath, "utf8")).version;
    }
    catch {
        packageVersion = "";
    }
    try {
        const updateLogText = await promises_1.default.readFile(updateLogPath, "utf8");
    ...
    try {
        latestYaml = await promises_1.default.readFile(latestYamlPath, "utf8");
    }
    catch {
        latestYaml = "";
    }
    checks.push(createReleaseCheck("version", "版本同步", packageVersion === version && updateLogVersion === version, packageVersion && updateLogVersion
        ? `package.json ${packageVersion}，更新记录 ${updateLogVersion}`
        : "未读取到 package.json 或更新记录版本"));
    checks.push(createReleaseCheck("installer", "安装包", await fileExists(installerPath), node_path_1.default.relative(repositoryRoot, installerPath)));
    checks.push(createReleaseCheck("blockmap", "增量索引", await fileExists(blockmapPath), node_path_1.default.relative(repositoryRoot, blockmapPath)));
    checks.push(createReleaseCheck("latest-yml", "更新清单", latestYaml.includes(`version: ${version}`) && latestYaml.includes(`AIstudy-Setup-${version}.exe`), "release/latest.yml"));
...
async function publishAppRelease(confirmVersion) {
    ...
    const installerPath = node_path_1.default.join(repositoryRoot, "release", `AIstudy-Setup-${status.version}.exe`);
    const blockmapPath = `${installerPath}.blockmap`;
    const latestYamlPath = node_path_1.default.join(repositoryRoot, "release", "latest.yml");
...
function registerDataHandlers() {
    ...
    electron_1.ipcMain.handle("developer-documents:load", async () => loadDeveloperDocumentDatabase());
    electron_1.ipcMain.handle("developer-documents:save", async (_event, documents) => saveDeveloperDocumentDatabase(documents));
    electron_1.ipcMain.handle("debug:knowledge-format-log", async (_event, entry) => appendKnowledgeFormatDebugLog(entry));
    electron_1.ipcMain.handle("debug:knowledge-format-log-path", async () => getKnowledgeFormatDebugLogPath());
    electron_1.ipcMain.handle("ai:chat", async (_event, request) => handleAiChatRequest(request));
    electron_1.ipcMain.handle("ai:system-context", async () => getSystemContextInfo());
    electron_1.ipcMain.handle("ports:status", async () => getManagedPortStatuses());
    electron_1.ipcMain.handle("ports:open-login-window", async (_event, platformId) => openManagedLoginWindow(platformId));
    electron_1.ipcMain.handle("ports:start-service", async (_event, platformId) => startManagedPortService(platformId));
    ...
    electron_1.ipcMain.handle("updates:release-status", async () => getAppReleaseStatus());
    electron_1.ipcMain.handle("updates:publish-release", async (_event, confirmVersion) => publishAppRelease(confirmVersion));
